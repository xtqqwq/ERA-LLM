...
def extract_trace(data_item, num_votes):
    ...
    for item in data_item:
        ...
        while str(i) in trace:
            i += 1
        if "chain_of_thought" in trace[str(i-1)]:
            res.append(trace[str(i-1)]["chain_of_thought"]["text"])
        elif trace[str(i-1)]["ost"] != {}:
            j = 1
            while str(j) in trace[str(i-1)]["ost"]:
                j += 1
            res.append(trace[str(i-1)]["ost"][str(j-1)])
