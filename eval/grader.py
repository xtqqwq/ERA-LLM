...
def math_equal_process_trl(param):
    # TODO: 
    from latex2sympy2_extended import NormalizationConfig
    from math_verify import LatexExtractionConfig, parse, verify
    import sympy
    content = '\\boxed{' + param[-2] + "}"
    sol = '\\boxed{' + param[-1] + "}"
    print(content, sol)
    gold_parsed = parse(
        sol,
    ...
    if len(gold_parsed) != 0:
        ...
        try:
            if isinstance(answer_parsed[0], float):
                answer_parsed[0] = sympy.Float(answer_parsed[0])
            elif isinstance(answer_parsed[0], int):
                answer_parsed[0] = sympy.Float(answer_parsed[0] * 1.0)
            if isinstance(gold_parsed[0], float):
                gold_parsed[0] = sympy.Float(gold_parsed[0])
            elif isinstance(gold_parsed[0], int):
                gold_parsed[0] = sympy.Float(gold_parsed[0] * 1.0)
            reward = float(verify(gold_parsed, answer_parsed, numeric_precision=2, float_rounding=2))
        except Exception as e:
            print(f"verify failed: content: {content}, answer: {answer_parsed}, gold: {gold_parsed}")
            reward = 0.0
    else:
