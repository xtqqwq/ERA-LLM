...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False):
    ...
    with ProcessPool(max_workers=1) as pool:
        # if 'math500' in data_name.lower() or 'gsm8k' in data_name.lower():
        #     future = pool.map(math_equal_process_trl, params, timeout=10)
        # else:
    ...
    # import pdb
    # pdb.set_trace()
    bool_array = np.array([x[0] for x in np.array(score_mat)])
    false_indices = np.where(bool_array == False)[0]
    if data_name.lower() == "gsm8k":
        evaluator = eval(f"GSM8KEvaluator()") 
    else:
        evaluator = eval(f"MATHEvaluator()") 
    # for fi in false_indices:
    #     if samples[fi]['gt'] not in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']:
    #         # print(f"=======Sample {fi}=======")
    #         # print("Solution: ", samples[fi]['code'][0][-100:])
    #         # print("Pred: ", samples[fi]['pred'])
    #         # print("GT: ", samples[fi]['gt'])
    #         answer_parsed = evaluator.extract_answer_from_model_completion(samples[fi]['code'][0])
    #         reward = evaluator.check_answers_equiv(answer_parsed, samples[fi]['gt'].replace("e", "10"))
    #         if reward == True:
    #             # import pdb
    #             # pdb.set_trace()
    #             a = 1
