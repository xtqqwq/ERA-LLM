...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    elif data_name in ["gpqa", "arc", "mmlu"]:
        gt_cot, gt_ans = None, example["reward_model"]["ground_truth"]
    elif data_name == "stg":
        gt_cot, gt_ans = None, example["answer"]
...
def parse_question(example, data_name):
    ...
    if data_name == "asdiv":
        ...
        prompts = example["prompt"][1]['content'].split("answering.\n\n")[-1].strip()
        question = prompts.replace("\n\nA)","\nAnswer Choices: (A)").replace("\nB)"," (B)").replace("\nC)", " (C)").replace("\nD)", " (D)").replace("\nE)", " (E)")
    elif data_name == "stg":
